...
function useTypewriter(words, speed = 90, pause = 2200, deleteSpeed = 55) {
  ...
  }, [text, wordIndex, isDeleting, isPausing, words, speed, pause, deleteSpeed])
...
const skills = {
  Frontend: { color: '#6366f1', items: ['React.js', 'JavaScript (ES6+)', 'HTML5', 'CSS3', 'Tailwind CSS', 'Bootstrap'] },
  'Frameworks & Tools': { color: '#a78bfa', items: ['Next.js', 'Firebase', 'Git', 'REST API Integration'] },
  'State Management': { color: '#38bdf8', items: ['Redux'] },
  Other: { color: '#34d399', items: ['Responsive Design', 'Component Architecture', 'API Handling'] },
}
...
const projects = [
  {
    ...
    description: 'Dynamic EMI calculator with real-time interest and tenure computation using React state management. Multi-step loan application forms with full validation logic.',
    tags: ['React', 'JavaScript', 'State Management', 'Forms'],
    github: 'https://github.com/sowndhar8',
  },
  {
    name: 'GitHub Profile Analyzer',
    description: 'Integrates the GitHub REST API to fetch and display user profiles and public repositories dynamically. Visualizes repository language distribution with interactive charts.',
    tags: ['React', 'GitHub API', 'Charts', 'REST API'],
    github: 'https://github.com/sowndhar8',
  },
  {
    name: 'E-Commerce Platform',
    description: 'Full-featured shopping application with product listings, cart, wishlist, checkout, and order tracking. Integrates Firebase for auth and data, Ant Design for UI components.',
  ...
  },
  {
    ...
    description: 'Multi-page company website with services, portfolio, and contact sections. Features smooth Framer Motion animations, EmailJS contact form, and Leaflet interactive map.',
  ...
  },
  {
    ...
    description: 'Corporate website with careers, products, services, and portfolio pages. Built with React Router for navigation and Framer Motion for polished page transitions.',
...
const certifications = [
  { name: 'Full Stack Web Developer – Java', issuer: 'Coding Ninjas', period: 'Nov 2022 – Oct 2023' },
]
    ...
    <nav className={`navbar${scrolled ? ' scrolled' : ''}`}>
      <div className="container nav-container">
        <a href="#home" className="nav-logo">
          <span className="logo-bracket">&lt;</span>ST<span className="logo-bracket">/&gt;</span>
        </a>
  ...
  return (
    <section id="home" className="hero">
      ...
      <div className="container hero-content">
        <p className="hero-greeting hero-anim" style={{ '--delay': '0.1s' }}>// Hello, world! I'm</p>
        <h1 className="hero-name hero-anim" style={{ '--delay': '0.25s' }}>{profile.name}</h1>
        ...
        <p className="hero-bio hero-anim" style={{ '--delay': '0.55s' }}>{profile.bio}</p>
        ...
        <div className="hero-socials hero-anim" style={{ '--delay': '0.85s' }}>
          ...
          <a href={`https://${profile.linkedin}`} target="_blank" rel="noreferrer" className="social-link" title="LinkedIn">
            <IconLinkedIn />
          </a>
...
function StatCard({ value, label, inView, delay }) {
  ...
  return (
    <div className="stat-card slide-right" style={{ transitionDelay: delay }}>
      <span className="stat-value">{inView ? `${count}${suffix}` : `0${suffix}`}</span>
    ...
    <section id="about" className="about">
      <div className="container" ref={ref}>
        ...
        <div className="about-grid">
          <div className={`about-text slide-left${inView ? ' visible' : ''}`} style={{ transitionDelay: '0.15s' }}>
            <p>
              I'm a <strong>Frontend Developer</strong> based in Coimbatore, India, with a strong passion
              for creating beautiful, performant web experiences. I hold an MCA from{' '}
              <strong>SRM Institute of Science and Technology</strong> with a 9.6 CGPA.
            </p>
            <p>
              Currently at <strong>Vineatz Technologies</strong>, I've built production-level
              financial platforms — designing reusable component systems, integrating REST APIs,
              and optimizing application performance for real users.
            </p>
            <p>
              Outside of work, I explore UI/UX design patterns, performance optimization, and
              system design concepts. I also enjoy troubleshooting computers at the hardware level.
            </p>
            <div className="about-badges">
              <span className="about-badge"><IconLocation /> {profile.location}</span>
              <span className="about-badge"><IconMail size={13} /> {profile.email}</span>
            </div>
          </div>
              ...
              <div className="skill-tags">
                {items.map((skill, j) => (
                  <span
                    ...
                    style={{ '--tag-color': color, animationDelay: `${0.2 + i * 0.1 + j * 0.05}s` }}
                  >
                    {skill}
                  </span>
                ))}
                ...
                {exp.projects.map((proj, j) => (
                  <div key={j} className="exp-project">
                    ...
                    <ul className="exp-points">
                      {proj.points.map((pt, k) => <li key={k}>{pt}</li>)}
                    </ul>
                  </div>
                ))}
              </div>
            ...
            >
              <div className="project-top">
                ...
                <div className="project-links">
                  <a href={proj.github} target="_blank" rel="noreferrer" className="project-link" title="GitHub">
                    <IconGitHub size={18} />
                  </a>
                  <a href={proj.github} target="_blank" rel="noreferrer" className="project-link" title="Open">
      ...
      <div className="container" ref={ref}>
        ...
        <div className="edu-grid">
          {education.map((edu, i) => (
            ...
            >
              <div className="edu-icon"><IconGraduation /></div>
              <div>
        ...
        <div className={`cert-section slide-up${inView ? ' visible' : ''}`} style={{ transitionDelay: '0.35s' }}>
          <p className="cert-heading">Certifications</p>
          {certifications.map((cert, i) => (
            <div key={i} className="cert-card">
              <span className="cert-icon"><IconCert /></span>
              <div>
                <p className="cert-name">{cert.name}</p>
                <p className="cert-meta">{cert.issuer} &middot; {cert.period}</p>
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <section id="contact">
      <div className="container" ref={ref}>
        ...
        <div className={`contact-wrapper slide-up${inView ? ' visible' : ''}`} style={{ transitionDelay: '0.15s' }}>
          <p className="contact-text">
            I'm currently open to frontend developer roles. Whether you have a project idea,
            a job opportunity, or just want to connect — my inbox is always open.
          </p>
          ...
          <div className="contact-links">
            ...
            <a href={profile.github} target="_blank" rel="noreferrer" className="contact-link-item">
              <IconGitHub size={16} /> github.com/sowndhar8
            </a>
            <a href={`https://${profile.linkedin}`} target="_blank" rel="noreferrer" className="contact-link-item">
              <IconLinkedIn size={16} /> LinkedIn
            </a>
            <span className="contact-link-item">
  ...
  return (
    <footer className="footer">
      <div className="container">
        <p>Designed &amp; built by <span className="footer-name">Sowndhar T M</span></p>
        <p style={{ marginTop: '0.25rem', fontSize: '0.8125rem' }}>© {new Date().getFullYear()}</p>
      </div>
    </footer>
  )
}
